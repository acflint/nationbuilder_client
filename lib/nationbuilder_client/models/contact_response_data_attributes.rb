...
module NationbuilderClient
  class ContactResponseDataAttributes
    ...
    def self.openapi_all_of
      [
      :'ContactReadOnlyAttributes',
      :'ContactReadWriteAttributes'
      ]
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NationbuilderClient::ContactResponseDataAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NationbuilderClient::ContactResponseDataAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'pc_in_cents')
        self.pc_in_cents = attributes[:'pc_in_cents']
      else
        self.pc_in_cents = 0
      end
    ...
    def hash
      [author_id, signup_id, contact_status, contact_method, broadcaster_id, content, pc_in_cents, path_id, path_step_id, created_at].hash
    end
