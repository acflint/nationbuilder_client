...
module NationbuilderClient
  class MembershipResponseDataAttributes
    ...
    # When the membership was suspended.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NationbuilderClient::MembershipResponseDataAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NationbuilderClient::MembershipResponseDataAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [grace_period_num_time_periods, grace_period_time_period_type, expires_on, membership_type_id, signup_id, status, status_reason, started_at, created_at, suspended_at, updated_at].hash
    end
